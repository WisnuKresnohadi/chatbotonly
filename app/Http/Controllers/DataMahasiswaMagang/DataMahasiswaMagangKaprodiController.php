...
class DataMahasiswaMagangKaprodiController extends DataMahasiswaMagangController
{
    ...
    public function indexKaprodi(Request $request) {
        ...
        $user = auth()->user();
        $data['kaprodi'] = $user->dosen;

        $data['dosen'] = Dosen::whereHas('user')->where('id_prodi', $data['kaprodi']->id_prodi)->get();
    ...
    public function assignPemAkademik(Request $request) {
        ...
        $request->validate([
            ...
            'dosen_pembimbing' => ['required', function ( $attribute, $value, $fail ) use ($dosen) {
                $id_prodi = $dosen->id_prodi;
                $dosen = Dosen::where('nip', $value)->where('id_prodi', $id_prodi)->first();
                if (!$dosen) {
                    $fail('Dosen tidak ditemukan');
                }
            }]
        ], [
        ...
        try {
            ...
            MhsMagang::whereHas('pendaftaran', function ($q) use ($dosen) {
                $q->whereHas('mahasiswa', function ($q2) use ($dosen) {
                    $q2->where('id_prodi', $dosen->id_prodi);
                });
            })->whereIn('id_pendaftaran', $request->id_pendaftaran)->update([
