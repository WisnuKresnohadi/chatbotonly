@if (count($profesional) > 0)
    <ul class="timeline ms-2 mb-0">
        @foreach ($profesional->take(2) as $key => $item)
            <li
                class="timeline-item timeline-item-transparent ps-4 {{ count($profesional) == $key + 1 ? 'border-0' : '' }}">
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <div class="d-flex justify-content-end">
                            <a class="cursor-pointer mx-1 text-warning" onclick="editData($(this))"
                                data-target-modal="modalTambahPengalaman" data-id="{{ $item->id_experience }}">
                                <i class="ti ti-edit"></i>
                            </a>
                            <a class="cursor-pointer mx-1 text-danger" onclick="deleteData($(this))"
                                data-function="afterDeleteExperience"
        ...
        @endforeach
        <div id="collapsePengalaman" class="accordion-collapse collapse" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
            <div class="mb-3">
                @foreach ($profesional->skip(2) as $key => $item)                                
                <li
                    class="timeline-item timeline-item-transparent ps-4 {{ count($profesional) == $key + 1 ? 'border-0' : '' }}">
                    <span class="timeline-point timeline-point-primary"></span>
                    <div class="timeline-event pe-4">
        ...
        @if (count($profesional) > 2)
            <button class="btn btn-outline-primary w-100 btn-collapse" type="button" data-bs-toggle="collapse"
                data-bs-target="#collapsePengalaman" aria-expanded="false" aria-controls="collapsePengalaman">
                Selengkapnya
            </button>
        @endif
    </ul>
@else
    ...
    <div class="sec-title mt-5 mb-4 text-center">
        <h4>Anda belum menambahkan riwayat pengalaman</h4>
    </div>
@endif
