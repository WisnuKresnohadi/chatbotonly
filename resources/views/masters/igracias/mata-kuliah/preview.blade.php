...
@section('page_style')
    <style>
        ...
        #table-master-matakuliah-data-duplikat-baru-tab-preview {
            border-collapse: collapse;
        }

        #table-master-matakuliah-data-duplikat-baru-tab-preview th,
        #table-master-matakuliah-data-duplikat-baru-tab-preview td {
            border: none;
            box-shadow: none;
        }
        ...
        /* #table-master-matakuliah-data-duplikat-baru-tab-preview tbody tr:hover {
...
@endsection
@section('content')
    <form name="data-import-failed" id="data-import-failed" method="POST" action="{{ route('igracias.matakuliah.download_failed_data') }}">
        @csrf
        <input type="hidden" name="failedData" id="failedData" value="{{ json_encode($data['failedData']) }}">
    </form>
    <form class="default-form" id="storeImport" name="storeImport" method="POST" action="{{ route('igracias.matakuliah.store_import') }}" function-callback="afterAction">
        @csrf
        <div class="row">
                ...
                <div class="tab-content" id="myTabContent">
                    ...
                    <div class="tab-pane fade show active" id="new-data-tab-pane" role="tabpanel"
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped border" id="table-master-matakuliah-data-baru-tab-preview">
                                <thead>
                                ...
                                <tbody>
                                    @foreach ($data['newData'] as $index => $mk)
                                        <tr>
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td>
                                                {{ $mk['kode_mk'] }}
                    ...
                    <div class="tab-pane fade" id="duplicate-data-tab-pane" role="tabpanel"
                        ...
                        <div class="rounded table-responsive d-flex flex-column">
                            ...
                            <table class="table border nowrap" id="table-master-matakuliah-data-duplikat-baru-tab-preview">
                                <thead>
                                    <tr>
                                        <th colspan="5" class="border-bottom">DATA BARU</th>
                                        <th colspan="4" class="border-start border-bottom">DATA LAMA</th>
                                    </tr>
                                    <tr style="text-align: start; background-color: white;">
                                        ...
                                        <th>NAMA MATAKULIAH</th>
                                        <th>SKS</th>             
                                        <th class="border-start">KODE MATAKULIAH</th>
                                        <th style="width: 10rem;">NAMA MATAKULIAH</th> 
                                        <th>SKS</th>                                                                                  
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($data['duplicatedData'] as $index => $mk)
                                        @php
                                            $mkExisting = $mk['existing'];
                                            $mkNew = $mk['new'];
                                        @endphp
                                        <tr>
                                            <td class="border input-check" style="background-color: white"><input
                                                    type="checkbox" name="duplicatedData" id="data{{ $index + 1 }}"
                                                    class="input-check checkbox" value="{{ json_encode($mkNew) }}">
                                            </td>
                                            ...
                                            <td
                                                class="{{ isset($mk['differences']['namamk']) ? 'onNewDiff' : 'onNewSame' }}">
                                                {{ $mkNew['namamk'] }}
                                            </td>
                                            <td
                                                class="{{ isset($mk['differences']['sks']) ? 'onNewDiff' : 'onNewSame' }}">
                                                {{ $mkNew['sks'] }}
                                            </td>                                            
                                            <td class="onNewSame">
                                                {{ $mkExisting['kode_mk'] }}
                    ...
                    <div class="tab-pane fade" id="failed-data-tab-pane" role="tabpanel"
                        ...
                        <div class="card-datatable table-responsive" style="width: 100%;">
                            ...
                            <table class="table table-striped border" id="table-master-matakuliah-data-gagal-tab-preview"
                                style="width: 100%;">
                                <thead>
                                    <tr style="text-align: start;">
                                        <th>NO</th>
                                ...
                                <tbody>
                                    @foreach ($data['failedData'] as $mkError)
                                        <tr>
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td class="{{ $mkError['kode_mk_error'] ? 'onerror' : 'onsuccess' }}">
                                                @if ($mkError['kode_mk_error'])
                                                    <h6>{{ $mkError['kode_mk_error'] }}</h6>
                                                @else
                                                    {{ $mkError['kode_mk'] }}
                                                @endif
                                            </td>
                                            <td class="{{ $mkError['namamk_error'] ? 'onerror' : 'onsuccess' }}">
                                                @if ($mkError['namamk_error'])
                                                    <h6>{{ $mkError['namamk_error'] }}</h6>
                                                @else
                                                    {{ $mkError['namamk'] }}
                                                @endif
                                            </td>
                                            <td class="{{ $mkError['sks_error'] ? 'onerror' : 'onsuccess' }}">
                                                @if ($mkError['sks_error'])
                                                    <h6>{{ $mkError['sks_error'] }}</h6>
                                                @else
                                                    {{ $mkError['sks'] }}
                                                @endif
                                            </td>                                          
                                            <td class="">
                                                @isset($mkError['messages'])
                                                    {!! $mkError['messages'] !!}
                                                @endisset
                                            </td>
                                        </tr>
...
@section('page_script')
    <script>
        $(document).ready(function() {
            $('#table-master-matakuliah-data-baru-tab-preview').DataTable({
                
            });
            $('#table-master-matakuliah-data-duplikat-baru-tab-preview').DataTable({
                fixedColumns: {
                    left: 1
                },
                paging: false,
            ...
            $('#table-master-matakuliah-data-gagal-tab-preview').DataTable({
